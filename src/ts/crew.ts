...
// Btns
const circleBtns = document.querySelectorAll('.memberBtn');
// personal info
const crewRole: HTMLHeadElement = document.querySelector(
	'.members__left--position'
)!;
const memberName: HTMLHeadElement = document.querySelector(
	'.members__left--name'
)!;
const description: HTMLParagraphElement =
	document.querySelector('.description')!;
const memberImg: HTMLImageElement = document.querySelector(
	'.members__right-img'
)!;
...
// 20.02.2024 updated code to toggle active class on buttons
function activeBtn(this: HTMLButtonElement) {
	closeActiveBtn();
	this.classList.toggle('active');
}

function closeActiveBtn() {
	circleBtns.forEach(btn => btn.classList.remove('active'))
}

circleBtns.forEach((item) => item.addEventListener('click', activeBtn));
